...
# %% auto 0
__all__ = ['MixedDataLoader', 'MixedDataLoaders', 'get_mixed_dls', 'get_test_mixed_dls']
...
class MixedDataLoader():
    def __init__(self, *loaders, device=None, bs=None, path='.', shuffle=False ):
...
def get_mixed_dls(*dls, shuffle_train=None, shuffle_valid=None, device=None, bs=None, **kwargs):
    _mixed_train_dls = []
    _mixed_valid_dls = []
    for dl in dls:
    ...
    # bs = ifnone(bs, min([dl.bs for dl in [mixed_train_dl, mixed_valid_dl]]))
    mixed_dls = MixedDataLoaders(mixed_train_dl, mixed_valid_dl, device=device)
    return mixed_dls


def get_test_mixed_dls(*dls, shuffle_train=None, shuffle_valid=None, device=None, bs=None, **kwargs):
    _mixed_test_dls = []
    for dl in dls:
        _mixed_test_dls.append(dl)
        if device is None: device = dl.device
    mixed_test_dl = MixedDataLoader(*_mixed_test_dls, **kwargs)
    mixed_dls = MixedDataLoaders(mixed_test_dl, device=device)
